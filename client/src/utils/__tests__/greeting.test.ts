    ...
    [defaultGreetingSlots, ...Object.values(greetingSlotsByDay)].forEach((slots) => {
      slots?.forEach((slot) => {
        ...
        if (slot.namedKey) {
          ...
          expect(translationEn[slot.namedKey]).toContain('{{name}}');
        }
      });
    });
  });
...
describe('getGreetingKey', () => {
  ...
  it('keeps the anonymous key for slots without a personalized variant', () => {
    const wednesdayMorning = dateForDay(3, 9);
    expect(getGreetingSlot(wednesdayMorning).namedKey).toBeUndefined();
    expect(getGreetingKey(wednesdayMorning, true)).toBe('com_ui_greeting_early_bird');
  });
});
