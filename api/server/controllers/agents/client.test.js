...
describe('AgentClient - resumeCompletion content protection', () => {
  const makeContext = (filters) => ({
    ...
    buildActivityLabelWiring: jest.fn(),
  ...
  it('blocks checkpoint user content before rebuilding the run', async () => {
    const getTuple = jest.fn().mockResolvedValue({
      checkpoint: {
        channel_values: {
  ...
  it('blocks checkpoint-only content from the active generation namespace', async () => {
    ...
    mockGetAgentCheckpointer.mockResolvedValue({ getTuple });
    const context = makeContext({
      messages: {
        pii: {
          fields: ['text'],
    ...
    context.checkpointNamespace = 'generation-1000';
